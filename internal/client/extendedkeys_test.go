...
func TestKittyNegotiate(t *testing.T) {
	cases := []struct {
		...
		{"enable from legacy", 0, 1, true, "\x1b[<1u\x1b[>1u", 1},
		{"disable to legacy", 1, 0, true, "\x1b[<1u", 0},
		{"change flags pops then pushes", 1, 5, true, "\x1b[<1u\x1b[>5u", 5},
		{"no change still re-asserts (drift repair)", 1, 1, true, "\x1b[<1u\x1b[>1u", 1},
		{"legacy to legacy still pops (stale entry)", 0, 0, true, "\x1b[<1u", 0},
		{"option off ignores want", 0, 1, false, "\x1b[<1u", 0},
